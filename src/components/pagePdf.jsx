...
const PagePdf = (props) => {
    ...
    return ( 
        <div style={{height:'53rem',backgroundColor:'rgb(160, 166, 165)'}}>
            <HeadBar></HeadBar>  
            <div>
            <div className="position-relative mt-4">
            <div className="mb5 position-absolute top-0 start-0"> <Link to='/Education'><button className='btn btn-primary ' type="button">previos</button></Link></div>
                 <div className="mb5 position-absolute top-0 end-0"><button className="btn btn-primary" onClick={printDocument}>Download as pdf</button></div>
            </div>
           
        <div style={{width:'40%', height:'750px',marginLeft:'30%',backgroundColor:'white'}} className='row '  id="divToPrint" ref={inputRef}>
    <div className="col-4" style={{backgroundColor:props.colorValue}}>
        <div>
            <br></br>
    ...
    <div class="col-8">
      ...
      {lastName.map(ev=>ev=<span style={{fontSize:'20px'}} className="fw-bold">{lastName[lastName.indexOf(ev)]}</span>)}
        ...
        <p>
             {props.arrAboutMeValues.map(ev=>ev=<span style={{fontSize:'10px'}} className="fw-bold justify-content-center">{props.arrAboutMeValues[props.arrAboutMeValues.indexOf(ev)]} </span>)} 
        </p>
        </div>
        </div>
        <hr></hr>
